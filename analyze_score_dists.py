...
if __name__ == '__main__':
	...
	schizophrenia_multi_pop_studies = [
		'GCST001851', 	#AFR,EAS,EUR
		'GCST004946',   #EAS,EUR
		'GCST009337',   #EAS,EUR
	]
	...
	studies = schizophrenia_studies + schizophrenia_multi_pop_studies
	...
	study_to_sample_pop = {
		...
		'GCST004946': 'EAS,EUR',   #EAS,EUR
		'GCST009337': 'EAS,EUR',   #EAS,EUR
		'GCST001851': 'AFR,EAS,EUR',   #AFR,EAS,EUR
	}
	...
	# Add Fst
	df['pop_Fst'] = df.apply(lambda s: 0. if ',' in s.sample_pop else Fst_dict[ s['pop'] ][ s['sample_pop'] ], axis=1)
	df.loc[df['pop_Fst'] < 0, 'pop_Fst'] = 0.
	df['super_pop_Fst'] = df.apply(lambda s: 0. if ',' in s.sample_pop else Fst_dict[ s['super_pop'] ][ s['sample_pop'] ], axis=1)
	...
	# Box plots
	gdf = df.groupby(['pop', 'STUDY ACCESSION', 'sample_pop', 'super_pop']).mean().reset_index()
	...
	fig, axs = plt.subplots(4, n_unique)
	fig.suptitle("Schizophrenia GWAS Scores")
	...
	for i, study in enumerate(df['STUDY ACCESSION'].unique()):
		...
		sample_pop = study_df.iloc[0].sample_pop
		study_res['sample_pop'].append(sample_pop)
		...
		# Kruskal–Wallis test
		s, p = stats.kruskal(*pop_scores)
		study_res['K-W stat'].append(s)
		study_res['K-W p-val'].append(p)

		# Conover-Iman posthoc
		r = sp.posthoc_conover(study_df, val_col='scaled_score', group_col='super_pop', p_adjust='holm')
		# Plot results
		sp.sign_plot(
			r, 
			xticklabels=True, 
			yticklabels=True, 
			ax=axs[1, i], 
			cbar_ax_bbox=[0.925, 0.55, 0.02, 0.15], 
			cmap=['1', '#9BCE8D', '#A43D47', '#EE856D', '#F5C0A3']
		)
		...
		# axs[2, i].set_title("B-F p-val = {:.3}".format(p))
		axs[2, i].set_xlabel(None)

		# Plot relationship with Fst
		if ',' in sample_pop:
			study_res['Fst & variance corr'].append(0.)
			study_res['Fst & variance corr p-val'].append(1.)
		else:
			sns.scatterplot(data=study_df, x='pop_Fst', y='pop_scaled_score_variance',
						hue='super_pop', ax=axs[3, i], legend=False)
		...
		if i == 5:
			axs[3, i].set_xlabel("Fst w/ sample pop")
		else:
			axs[3, i].set_xlabel("")
		# axs[3, i].set_title("Pearson r = {:.3}\np-val = {:.3}".format(r, p))

		if i > 0:
			axs[0, i].set_ylabel(None)
			axs[2, i].set_ylabel(None)
			axs[3, i].set_ylabel(None)
		else:
			axs[1, i].set_ylabel("Originate From\nSame Distribution")
			axs[2, i].set_ylabel("Equality of Super\nPop. Variances")
			axs[3, i].set_ylabel("Standardized Score\nVariance")

	all_studies_df = pd.DataFrame(study_res)
	all_studies_df['K-W corrected p-val'] = multipletests(
		all_studies_df['K-W p-val'],
		alpha=.01,
		method='holm')[1]
